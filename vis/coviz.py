...
from glanvup.continents import africa_coast, asia_coast, europe_coast, south_coast, north_coast
import warnings
...
def copy_shapefiles(iso3, name_pattern, continent):
    ...
    root_folder = os.path.join(DATA_RESULTS, iso3, 'pop_cozard_coverage_poverty')
    output_folder = os.path.join(BASE_PATH, 'global_results', 'coast_shapefiles', continent) 
...
def collect_shapefiles(continent):
    ...
    '''
    shapefiles = []
    root_folder = os.path.join(BASE_PATH, 'global_results', 'coast_shapefiles', continent)
    ...
    for folder_path, _, files in os.walk(root_folder):
        ...
        for file in files:
            ...
            print('Collecting {}.'.format(country_iso))
...
def plot_map(gdf, continent):
    ...
    path = os.path.join(DATA_VIS, '{}_coastal_map.png'.format(continent))
    plt.savefig(path)
    #plt.show()
...
if __name__ == '__main__':
    ...
    name_pattern = 'GSM_nuncoast_historical_wtsub_hist_rp0100_'
    for iso in isos:
        
        copy_shapefiles(iso, name_pattern, 'Europe')

    global_shapefiles = collect_shapefiles('Europe')
    combined_gdf = combine_geodataframes(global_shapefiles)
    plot_map(combined_gdf, 'Europe')